...
const ZynDemo = {
  ...
  init() {
    ...
    window.addEventListener("message", (e) => this.handleEmbedMessage(e));
  ...
  // Embedded in another page (Tracklathe puts this editor in a tab), the
  // editor IS that page's selected instrument: every change to the seed,
  // octave or volume is posted up as it happens, and the page can post a
  // state down to show. `embedQuiet` stops a state we were just given from
  // being echoed straight back.
  postEmbedState() {
    if (window.parent === window || this.embedQuiet) return;
    try {
      window.parent.postMessage({
        type: "zyn-state",
        seed: this.randInstrumentSeed,
        octave: parseInt(document.getElementById("octaveSelect").value) || 0,
        volume: this.volume,
      }, "*");
    } catch (e) {}
  },
  ...
  handleEmbedMessage(e) {
    const d = e.data;
    if (!d || d.type !== "zyn-load") return;
    const seed = Number(d.seed);
    if (!Number.isFinite(seed)) return;
    this.embedQuiet = true;
    try {
      if (d.octave !== undefined) document.getElementById("octaveSelect").value = String(d.octave);
      if (d.volume !== undefined) this.setVolume(Number(d.volume));
      document.getElementById("instrumentSeedInput").value = seed;
      this.activePresetIndex = null;
      this.updateInstrumentAndPushState(seed);
    } finally {
      this.embedQuiet = false;
    }
  },
